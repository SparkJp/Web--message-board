...
public class myDB {
	...
	public void getConnection() throws ClassNotFoundException, SQLException {
		...
		String sqlurl = "jdbc:sqlserver://localhost:1433;DatabaseName=tte";
		String user = "zhang";
		String ps = "123456";
	...
	public boolean checkLogin(String id, String password) throws SQLException {
		...
		String sqlq = "select * from dbo.usser where id= '" + id + "' and password= '" + password + "'";
	...
	public int getStatus(String id) throws SQLException {
		Statement stmt = conn.createStatement();
		String sqlq = "select status from usser where " + "id = " + "'" + id + "'";
		ResultSet rs = stmt.executeQuery(sqlq);
		if (rs.next()) {
			return rs.getInt(1);
		} else
	...
	public int saveContent(String content, String title, String userid) throws SQLException {
		...
		String sql_save = "insert into message" + "(id , title , userid , content , createtime)"
				+ "values(? , ? , ? , ? , ?)";
	...
	public List<Map<String, String>> getUserList() throws SQLException {
		...
		String sql_select = "select * from usser";
	...
	public List<Map<String, Object>> getMainPage(int row, int offset) throws SQLException {
		List records = new ArrayList();
		String sql_select = "select message.id as id, username , title , message.createtime , userid as createtime from message , usser where message.userid=usser.id order by message.createtime";
	...
	public List<Map<String, String>> getSubPage(String pid) throws SQLException {
		...
		String sql_select = "select username , title , content , format(message.createtime,'yyyy-MM-dd hh:mm:ss ')"
				+ "as sec_createtime from message , usser where message.id = '" + pid + "'"
				+ "and message.userid=usser.id";
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt.executeQuery(sql_select);
		ResultSetMetaData rsmd = rs.getMetaData();
		int fcnt = rsmd.getColumnCount();
		...
		rs.close();
		stmt.close();
		sql_select = "select username , title , content , format(message.createtime,'yyyy-MM-dd hh:mm:ss ')"
				+ "as sec_createtime from message , ht , usser where ht.hid = '" + pid + "'" + " and message.id=ht.id"
				+ " and message.userid=usser.id";
	...
	public Long countUser(String sid) throws SQLException {
		String sql_select = "select count(id) as count from usser where id = " + "'" + sid + "'";
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt.executeQuery(sql_select);
		if (!rs.next())
			return (long) 0;
	...
	public int insertUser(String password, String sid, String username, String usertype) throws SQLException {
		String sql_save = "insert into usser" + "(id , password , username , status , createtime , usertype)"
				+ "values(? , ? , ? , ? , ? , ?  )";
		PreparedStatement pstmt = conn.prepareStatement(sql_save);
